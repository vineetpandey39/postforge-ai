...
function imagePrompt({ type, text, subline, stat, visual, pillarId, layoutIndex }) {
...
Composition: ${isCover
    ? 'extreme hook cover, massive curiosity, one dominant cinematic object, diagonal energy, danger/secret/reveal feeling, thumbnail-readable from far away'
    : 'follow-up carousel update slide, dense but organized visual metaphor, one clear hero object, supporting holograms/icons/charts, premium infographic energy without paragraphs'}.
...
export async function POST(request) {
  try {
    ...
    const prompts = [
      { index: 0, label: 'Cover', prompt: imagePrompt({ type: 'cover', text: cover_text || hook, subline: cover_subtext || '', visual: cover_visual_prompt, pillarId, layoutIndex: 0 }) },
      ...slides.map((slide, i) => ({
        index: i + 1,
